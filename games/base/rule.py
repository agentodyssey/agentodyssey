...
if TYPE_CHECKING:
    from games.base.env import AgentOdysseyEnv
    from games.base.agent import Agent
    from games.base.world import World
...
class DependencyTracker:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Export the dependency graph as a JSON-serialisable dict.

        Structure::

            {
                "edges": {
                    "pick up -> craft": [[5, 10], [5, 12]],
                    ...
                },
                "step_hints": {
                    "0":  ["..."],
        ...
        ``edges`` maps ``"src_rule -> dst_rule"`` to a sorted list of
        ``[src_step, dst_step]`` pairs.  Each pair records one action-level
        dependency: the action executed by *src_rule* at *src_step* produced
        a token later consumed / moved by *dst_rule* at *dst_step*.

        Duplicate step pairs within the same edge type are preserved (they
        represent distinct token-level dependencies).
        """
