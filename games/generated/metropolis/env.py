...
from games.generated.metropolis.world import World, Object, NPC, Path, Area, Place, Container
from games.generated.metropolis.agent import Agent, Inventory, Action
from tools.logger import get_logger
from games.generated.metropolis.rule import RuleResult, RuleContext, ActionRuleEngine, BaseStepRule, BaseActionRule, DependencyTracker
from games.generated.metropolis.rules.reward_functions import RewardFunction, RewardBreakdown, DefaultRewardFunction
import games.generated.metropolis.rules.action_rules as action_rules
import games.generated.metropolis.rules.step_rules as step_rules
...
class AgentOdysseyEnv(gym.Env):
    ...
    def get_all_valid_actions(self, agent: Agent) -> list[str]:
        ...
        has_catalyst = (
            int(agent.items_in_hands.get(catalyst_id, 0)) > 0
            or (agent.inventory.container and int(agent.inventory.items.get(catalyst_id, 0)) > 0)
            or any(
                int(self.world.container_instances[oid].inventory.get(catalyst_id, 0)) > 0
                for oid in agent.items_in_hands.keys()
                if oid in self.world.container_instances
            )
        )
        if has_catalyst:
            for obj_id in hand_objects:
                if obj_id in self.world.writable_instances:
                    writable_inst = self.world.writable_instances[obj_id]
                    if (writable_inst.text or "").strip():
                        w_name = obj_id_to_name.get(obj_id, writable_inst.name)
                        actions["invoke law"].append(f"invoke law {w_name}")

        # --- File Objection ---
        active_combats = self.curr_agents_state.get("active_combats", {}).get(agent.id, {})
        if active_combats:
            for npc_id in active_combats:
                if npc_id in self.world.npc_instances:
                    npc_inst = self.world.npc_instances[npc_id]
                    actions["file objection"].append(f"file objection {npc_inst.name}")

        # --- Appeal Verdict ---
        pending_death = self.curr_agents_state.get("pending_death", {}).get(agent.id, False)
        if agent.hp <= 0 or pending_death:
            # check if agent has any coins at all
            coin_id = "obj_coin"
            appeal_coins = int(agent.items_in_hands.get(coin_id, 0))
            if agent.inventory.container:
                appeal_coins += int(agent.inventory.items.get(coin_id, 0))
            for ci in held_containers:
                appeal_coins += int(ci.inventory.get(coin_id, 0))
            if appeal_coins > 0:
                actions["appeal verdict"].append("appeal verdict")
...
"""
    ...
    def reset(self, from_config: bool = False) -> Dict[str, str]:
        if not from_config:
            ...
            self.curr_agents_state.setdefault("active_invocations", {})
            self.curr_agents_state.setdefault("pacified_npcs", {})
            self.curr_agents_state.setdefault("objection_skip_turn", {})
            self.curr_agents_state.setdefault("appeal_verdict_used", {})
            self.curr_agents_state.setdefault("pending_death", {})
            for agent in self.agents:
                ...
                self.curr_agents_state["objection_skip_turn"].setdefault(agent.id, False)
                self.curr_agents_state["appeal_verdict_used"].setdefault(agent.id, False)
                self.curr_agents_state["pending_death"].setdefault(agent.id, False)
            ...
            # nightly law rewrite tracking
            self.curr_agents_state.setdefault("nightly_law_rewrite", {
                "last_night_step": 0,
                "night_count": 0,
            })

            # legal paradox tracking
            self.curr_agents_state.setdefault("legal_paradox", {})

            # contempt of court tracking
            self.curr_agents_state.setdefault("contempt_of_court", {})

            # binding precedent tracking
            self.curr_agents_state.setdefault("binding_precedents", {
                "area_precedents": {},
                "overturned_types": {},
                "weakened_npcs": {},
                "boosted_npcs": {},
            ...
            # contraband suspicion tracking
            self.curr_agents_state.setdefault("contraband_suspicion", {})

            # trial proceedings tracking
            self.curr_agents_state.setdefault("trial_proceedings", {})
    ...
    def step(self, action_strs: Dict[str, str]):
        ...
        for agent in self.agents:
            # enforce skip-turn penalty from overruled objections
            skip_turn = self.curr_agents_state.get("objection_skip_turn", {}).get(agent.id, False)
            if skip_turn:
                self.curr_agents_state["objection_skip_turn"][agent.id] = False
                res.add_feedback(
                    agent.id,
                    f"{self.person_verbalized['subject_pronoun']} {self.person_verbalized['to_be_conjugation']} "
                    f"stunned from the overruled objection and cannot act this turn.\n"
                )
                action_strs[agent.id] = "wait"
        ...
        # record last actions for contempt-of-court tracking
        contempt = self.curr_agents_state.setdefault("contempt_of_court", {})
        for agent in self.agents:
            if agent.id not in contempt:
                contempt[agent.id] = {
                    "idle_area": self.curr_agents_state["area"].get(agent.id),
                    "idle_turns": 0,
                    "last_action": None,
                    "hp_reduction_applied": 0,
                }
            parsed_action, _ = self.parse_action(agent, action_strs.get(agent.id, "wait"))
            contempt[agent.id]["last_action"] = parsed_action if parsed_action else "wait"
