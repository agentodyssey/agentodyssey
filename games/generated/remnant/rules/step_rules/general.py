...
class ActiveAttackStepRule(BaseStepRule):
    ...
    def apply(self, ctx: RuleContext, res: RuleResult) -> None:
        ...
        base_attack_prob, max_attack_prob = 0.05, 0.25
...
class DayCycleStepRule(BaseStepRule):
    ...
    def _set_step_modifiers(self, env, period: str) -> None:
        ...
        if period == "dangerous_night":
            env._step_modifiers["ambush_prob_additive"] = 0.30
...
class EnemyRespawnStepRule(BaseStepRule):
    ...
    def __init__(self) -> None:
        super().__init__()
        self._default_interval = (25, 45)  # steps between respawn waves
...
class SoundscapePressureStepRule(BaseStepRule):
    ...
    def __init__(self) -> None:
        ...
        self._scout_spawn_prob = 0.03
        self._hush_steps_required = 4
...
class ShiftingPathsStepRule(BaseStepRule):
    name = "shifting_paths_step"
    description = "Occasionally and temporarily lock or unlock paths based on soundscape noise and moon phase, then revert."
    priority = 6

    def __init__(self) -> None:
        super().__init__()
        self._interval_min = 6
        self._interval_max = 12
        self._duration_min = 3
        self._duration_max = 6
        self._hush_eps = 0.05
        self._loud_threshold = 3.0

    def _ensure_state(self, world) -> dict:
        aux = world.auxiliary
        sp = aux.setdefault("shifting_paths", {})
        sp.setdefault("active", {})  # key "u|v" (sorted) -> {"expires": int, "prev_locked": bool}
        if "next_step" not in sp:
            sp["next_step"] = 0
        return sp
    ...
    @staticmethod
    def _has_agent_in_area(env, area_id: str) -> bool:
        for a in env.agents:
            if env.curr_agents_state["area"].get(a.id) == area_id:
                return True
        return False

    @staticmethod
    def _area_has_other_unlocked_exit(world, area_id: str, exclude_neighbor_id: str) -> bool:
        area = world.area_instances.get(area_id)
        if area is None:
            return False
        for nid, path in (getattr(area, "neighbors", {}) or {}).items():
            if nid == exclude_neighbor_id:
                continue
            if getattr(path, "locked", False) is False:
                return True
        return False

    @staticmethod
    def _quest_gated(world, path) -> bool:
        oid = getattr(path, "object_to_unlock", None)
        if not oid:
            return False
        try:
            base = get_def_id(oid)
            ...
            base = str(oid)
        odef = (getattr(world, "objects", {}) or {}).get(base)
        return bool(getattr(odef, "quest", False)) if odef is not None else False

    def _neighbors_pair(self, world, u: str, v: str):
        au = world.area_instances.get(u)
        av = world.area_instances.get(v)
        if au is None or av is None:
            return None, None, None
        pu = (getattr(au, "neighbors", {}) or {}).get(v)
        pv = (getattr(av, "neighbors", {}) or {}).get(u)
        return au, av, (pu, pv)

    def _phase_bias(self, time_period: str, lock_prob: float, unlock_prob: float) -> tuple[float, float]:
        if time_period in ("midnight_danger", "night_danger"):
            lock_prob *= 1.3
            unlock_prob *= 0.8
        elif time_period in ("dawn_bonus", "midday_calm", "afternoon_calm", "evening_bonus"):
            unlock_prob *= 1.3
            lock_prob *= 0.8
        return lock_prob, unlock_prob

    def _noise_bias(self, world, u: str, v: str, lock_prob: float, unlock_prob: float) -> tuple[float, float]:
        ss = (world.auxiliary or {}).get("soundscape", {}) or {}
        noise = ss.get("noise", {}) or {}
        loud_flags = ss.get("loud_flags", {}) or {}
        nu = float(noise.get(u, 0.0))
        nv = float(noise.get(v, 0.0))
        is_loud = bool(loud_flags.get(u, False) or loud_flags.get(v, False) or max(nu, nv) >= self._loud_threshold)
        is_hush = (nu <= self._hush_eps and nv <= self._hush_eps)
    ...
    def _emit_feedback_and_event(self, env, world, res, u: str, v: str, mode: str, expires: int) -> None:
        u_name = world.area_instances[u].name if u in world.area_instances else u
        v_name = world.area_instances[v].name if v in world.area_instances else v

        for agent in env.agents:
            aid = env.curr_agents_state["area"].get(agent.id)
            if aid == u:
                if mode == "lock":
                    res.add_feedback(agent.id, f"A path toward {v_name} slams shut for a short while.\n")
                else:
                    res.add_feedback(agent.id, f"A path toward {v_name} opens briefly.\n")
            elif aid == v:
                if mode == "lock":
                    res.add_feedback(agent.id, f"A path toward {u_name} slams shut for a short while.\n")
                else:
                    res.add_feedback(agent.id, f"A path toward {u_name} opens briefly.\n")

        ev_type = "path_temporarily_locked" if mode == "lock" else "path_temporarily_unlocked"
        res.events.append(Event(
            type=ev_type,
            agent_id="env",
            data={"from_area": u, "to_area": v, "expires_step": int(expires)},
        ))

    def _emit_expire_feedback(self, env, world, res, u: str, v: str) -> None:
        u_name = world.area_instances[u].name if u in world.area_instances else u
        v_name = world.area_instances[v].name if v in world.area_instances else v
        ...
        for agent in env.agents:
            aid = env.curr_agents_state["area"].get(agent.id)
            if aid == u:
                res.add_feedback(agent.id, f"A shifting path to {v_name} reverts.\n")
            elif aid == v:
                res.add_feedback(agent.id, f"A shifting path to {u_name} reverts.\n")

        res.events.append(Event(
            type="path_shift_expired",
            agent_id="env",
            data={"from_area": u, "to_area": v},
        ))
    ...
    def apply(self, ctx: RuleContext, res: RuleResult) -> None:
        ...
        state = self._ensure_state(world)
        active = state["active"]

        # expire any active shifts
        to_remove: list[str] = []
        for key, meta in list(active.items()):
            try:
                u, v = key.split("|", 1)
            except Exception:
                to_remove.append(key)
                continue
            expires = int(meta.get("expires", -1))
            prev_locked = bool(meta.get("prev_locked", True))
            if int(env.steps) >= expires:
                au, av, paths = self._neighbors_pair(world, u, v)
                if au is not None and av is not None and paths is not None:
                    pu, pv = paths
                    if pu is not None:
                        pu.locked = prev_locked
                    if pv is not None:
                        pv.locked = prev_locked
                    self._emit_expire_feedback(env, world, res, u, v)
                to_remove.append(key)
        for key in to_remove:
            active.pop(key, None)

        # only occasionally trigger new shifts
        if int(env.steps) < int(state.get("next_step", 0)):
            return
        state["next_step"] = int(env.steps) + env.rng.randint(self._interval_min, self._interval_max)

        day_cycle = (world.auxiliary or {}).get("day_cycle", {}) or {}
        time_period = day_cycle.get("current_period", "")
        seen: set[str] = set()
        candidates: list[tuple[str, str]] = []
        for u, area in world.area_instances.items():
            for v, path in (getattr(area, "neighbors", {}) or {}).items():
                ...
                au, av, paths = self._neighbors_pair(world, u, v)
                if au is None or av is None or paths is None:
                    continue
                pu, pv = paths
                if pu is None or pv is None:
                    continue

                if (tutorial_active and (u == tutorial_area_id or v == tutorial_area_id)) or (u == spawn_area_id or v == spawn_area_id):
                    continue

                if self._quest_gated(world, pu) or self._quest_gated(world, pv):
                    continue
        ...
        shifts_applied = 0
        max_shifts = env.rng.randint(1, 2)

        for u, v in candidates:
            if shifts_applied >= max_shifts:
                break
            ...
            # compute probabilities influenced by noise + time period
            lock_prob = 0.10
            unlock_prob = 0.10
            lock_prob, unlock_prob = self._noise_bias(world, u, v, lock_prob, unlock_prob)
            lock_prob, unlock_prob = self._phase_bias(time_period, lock_prob, unlock_prob)

            want_mode: Optional[str] = None
            if pu.locked is False and env.rng.random() < lock_prob:
                # safety: do not lock if this would be the last open exit of an area that currently holds an agent
                block = False
                if self._has_agent_in_area(env, u):
                    if not self._area_has_other_unlocked_exit(world, u, exclude_neighbor_id=v):
                        block = True
                if self._has_agent_in_area(env, v):
                    if not self._area_has_other_unlocked_exit(world, v, exclude_neighbor_id=u):
                        block = True
                if not block:
                    want_mode = "lock"

            elif pu.locked is True and env.rng.random() < unlock_prob:
                want_mode = "unlock"
            ...
            key = self._edge_key(u, v)
            prev_locked = bool(pu.locked)
            expires = int(env.steps) + env.rng.randint(self._duration_min, self._duration_max)
            if want_mode == "unlock":
                if pu is not None:
                    pu.locked = False
                if pv is not None:
            ...
            active[key] = {"expires": expires, "prev_locked": prev_locked}
            self._emit_feedback_and_event(env, world, res, u, v, want_mode, expires)
            shifts_applied += 1
