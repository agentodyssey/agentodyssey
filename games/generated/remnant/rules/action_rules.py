import math
from games.generated.remnant.rule import BaseActionRule, RuleContext, RuleResult, Event
...
class LockpickRule(BaseActionRule):
    name = "action_lockpick"
    verb = "lockpick"
    param_min = param_max = 1
    params = ["area_name"]
    description = "Attempt to silently unlock a neighboring locked path without a key."

    def apply(self, ctx: RuleContext, res: RuleResult) -> None:
        env, world, agent = ctx.env, ctx.world, ctx.agent
        target_area_name = ctx.params[0]
        ...
        neighbor_id = None
        connecting_path = None
        for nid, path in current_area.neighbors.items():
            if world.area_instances[nid].name == target_area_name:
                neighbor_id = nid
                connecting_path = path
                break

        if neighbor_id is None or connecting_path is None:
            res.add_feedback(agent.id, f"Cannot lockpick {target_area_name}.\n")
            return

        if not getattr(connecting_path, "locked", False):
            res.add_feedback(agent.id, f"Failed to lockpick. The door to {target_area_name} is already unlocked.\n")
            return

        for oid, cnt in agent.items_in_hands.items():
            if cnt <= 0:
                continue
            if get_def_id(oid) == "obj_lockpick" or get_def_id(oid) == "obj_tension_wrench":
                break
        else:
            res.add_feedback(agent.id, f"Cannot lockpick {target_area_name}. No lockpick tool in hand.\n")
            return

        success_chance = 0.20 if "obj_lockpick" in agent.items_in_hands else 0.10

        roll = env.rng.random()
        if roll < success_chance:
            # success: unlock both directions, do not move
            connecting_path.locked = False
            if current_area_id in world.area_instances[neighbor_id].neighbors:
                world.area_instances[neighbor_id].neighbors[current_area_id].locked = False

            res.add_feedback(agent.id, f"{env.person_verbalized['subject_pronoun']} picked the lock to {target_area_name}.\n")
            res.events.append(Event(
                type="path_unlocked",
                agent_id=agent.id,
                data={"from_area": current_area_id, "to_area": neighbor_id},
            ))

            use_oid = "obj_lockpick" if "obj_lockpick" in agent.items_in_hands else "obj_tension_wrench"
            agent.items_in_hands[use_oid] -= 1
            if agent.items_in_hands[use_oid] <= 0:
                del agent.items_in_hands[use_oid]
            res.track_consume(agent.id, use_oid, 1, src=res.tloc("hand", agent.id))
        else:
            res.add_feedback(agent.id, f"{env.person_verbalized['subject_pronoun']} failed to pick the lock.\n")
            res.events.append(Event(
                type="lockpick_failed",
                agent_id=agent.id,
                data={"from_area": current_area_id, "to_area": neighbor_id},
            ))
            res.events.append(Event(
            ...
            # 25% chance agent takes 1, applying defense
            if env.rng.random() < 0.25:
                base_dmg = 1
                actual_dmg = max(1, base_dmg - int(getattr(agent, "defense", 0)))
                agent.hp -= actual_dmg

                res.add_feedback(agent.id, f"{env.person_verbalized['subject_pronoun']} took {actual_dmg} damage while attempting the lockpick.\n")
                res.events.append(Event(
                    type="agent_damaged",
                    agent_id=agent.id,
                    data={"damage": actual_dmg, "area_id": current_area_id, "reason": "lockpick_fail"},
                ))
...
class EatRule(BaseActionRule):
    ...
    def apply(self, ctx: RuleContext, res: RuleResult) -> None:
        ...
        if obj_name not in world.auxiliary["obj_name_to_id"]:
            res.add_feedback(agent.id, f"Cannot eat {obj_name}, not found in hand, inventory, held containers, or on the ground.\n")
            return
        ...
        is_consumable = getattr(obj_def, "usage", "") == "consume" or getattr(obj_def, "category", "") == "food" or hp_restore > 0
        if not is_consumable:
            res.add_feedback(agent.id, f"Cannot eat {obj_name}, it is not edible.\n")
            return

        if agent.hp >= agent.max_hp:
            res.add_feedback(agent.id, f"{env.person_verbalized['subject_pronoun']} am already at full health.\n")
            return
        ...
        if obj_id in agent.items_in_hands and agent.items_in_hands[obj_id] > 0:
        ...
        elif agent.inventory.container and base_id in agent.inventory.items and agent.inventory.items[base_id] > 0:
            agent.inventory.items[base_id] -= 1
            if agent.inventory.items[base_id] == 0:
                del agent.inventory.items[base_id]
            src_loc = res.tloc("container", agent.inventory.container.id)
            consumed_oid = base_id
            ...
            found = False
            for ci in held_containers:
                if base_id in ci.inventory and ci.inventory[base_id] > 0:
                    ci.inventory[base_id] -= 1
                    if ci.inventory[base_id] == 0:
                        del ci.inventory[base_id]
                    src_loc = res.tloc("container", ci.id)
                    consumed_oid = base_id
                    from_label = world.container_instances[ci.id].name if ci.id in world.container_instances else "container"
                    found = True
                    break
            if not found:
                if base_id in current_area.objects and current_area.objects[base_id] > 0:
                    current_area.objects[base_id] -= 1
                    if current_area.objects[base_id] == 0:
                        del current_area.objects[base_id]
                    src_loc = res.tloc("area", current_area.id)
                    consumed_oid = base_id
                    from_label = "ground"

        if src_loc is None or consumed_oid is None:
            res.add_feedback(agent.id, f"Cannot eat {obj_name}, not found in hand, inventory, held containers, or on the ground.\n")
            return
        ...
        res.add_feedback(
            agent.id,
            f"{env.person_verbalized['subject_pronoun']} ate {obj_name} from {from_label} and restored {restored} HP "
            f"(HP: {agent.hp}/{agent.max_hp}).\n"
        )
        res.events.append(Event(
            type="object_consumed",
            agent_id=agent.id,
            data={"obj_id": base_id, "from": from_label, "hp_restored": restored, "area_id": current_area.id},
        ))

class ThrowRule(BaseActionRule):
    name = "action_throw"
    verb = "throw"
    param_min = param_max = 2
    params = ["obj_name", "npc_name"]
    description = "Hurl an object in hand at an NPC in the same area, dealing instant damage without immediate counterattack."

    def apply(self, ctx: RuleContext, res: RuleResult) -> None:
        env, world, agent = ctx.env, ctx.world, ctx.agent
        obj_name, npc_name = ctx.params[0], ctx.params[1]

        if obj_name not in world.auxiliary.get("obj_name_to_id", {}):
            res.add_feedback(agent.id, f"Cannot throw {obj_name}, not found in hand.\n")
            return
        obj_id = world.auxiliary["obj_name_to_id"][obj_name]
        ...
        if obj_id in world.container_instances or obj_id in world.writable_instances:
            res.add_feedback(agent.id, f"Cannot throw {obj_name}, it cannot be thrown.\n")
            return
        base_id = get_def_id(obj_id)
        ...
        current_area = world.area_instances[env.curr_agents_state["area"][agent.id]]
        if npc_name not in world.auxiliary.get("npc_name_to_id", {}):
            res.add_feedback(agent.id, f"Cannot throw at {npc_name}, not found in current area.\n")
            return
        target_npc_id = world.auxiliary["npc_name_to_id"][npc_name]
        if target_npc_id not in current_area.npcs:
            res.add_feedback(agent.id, f"Cannot throw at {npc_name}, not found in current area.\n")
            return
        target_npc = world.npc_instances[target_npc_id]

        damage = max(1, int(getattr(obj_def, "attack_power", 0)))
        ...
        res.events.append(Event(
            type="object_thrown",
            agent_id=agent.id,
            data={"obj_id": obj_id, "npc_id": target_npc_id, "area_id": current_area.id},
        ))

        prev_hp = int(target_npc.hp)
        target_npc.hp = max(0, prev_hp - damage)
        res.add_feedback(
            agent.id,
            f"{env.person_verbalized['subject_pronoun']} threw {obj_name} at {npc_name}, dealing {damage} damage.\n"
        )
        res.events.append(Event(
            type="npc_hit_by_thrown",
            agent_id=agent.id,
            data={"obj_id": obj_id, "npc_id": target_npc_id, "damage": damage, "area_id": current_area.id},
        ))

        # aftermath of the thrown item: 70% lands intact, 30% shatters
        lands_intact = env.rng.random() < 0.7
        if lands_intact:
            current_area.objects[obj_id] = current_area.objects.get(obj_id, 0) + 1
            res.track_move(
                agent.id, obj_id, 1,
                src=res.tloc("hand", agent.id),
                dst=res.tloc("area", current_area.id),
            ...
            # salvage like disassemble
            ingredients = obj_def.craft_ingredients or {}
            salvage: dict[str, int] = {}
            for ing_id, req in ingredients.items():
                try:
                    req_amt = float(req)
                except Exception:
                    continue
                out = 0
                if req_amt >= 2:
                    out = max(1, math.floor(0.5 * req_amt))
                elif abs(req_amt - 1.0) < 1e-9:
                    out = 1 if env.rng.random() < 0.5 else 0
                else:
                    out = 0
                if out > 0:
                    salvage[ing_id] = int(out)

            for ing_id, cnt in salvage.items():
                if ing_id in world.objects and (world.objects[ing_id].category == "container" or world.objects[ing_id].usage == "writable"):
                    # create fresh instances for each salvage unit
                    if world.objects[ing_id].category == "container":
                        instance_list = world.container_instances
                        id_to_count = world.auxiliary.setdefault("container_id_to_count", {})
                    else:
                        instance_list = world.writable_instances
                        id_to_count = world.auxiliary.setdefault("writable_id_to_count", {})
                    id_to_count.setdefault(ing_id, 0)
                    for _ in range(cnt):
                        new_instance = world.objects[ing_id].create_instance(id_to_count[ing_id])
                        instance_list[new_instance.id] = new_instance
                        id_to_count[ing_id] += 1
                        world.auxiliary["obj_name_to_id"][new_instance.name] = new_instance.id
                        current_area.objects[new_instance.id] = current_area.objects.get(new_instance.id, 0) + 1
                        res.track_spawn(agent.id, new_instance.id, 1, dst=res.tloc("area", current_area.id))
                else:
                    current_area.objects[ing_id] = current_area.objects.get(ing_id, 0) + cnt
                    res.track_spawn(agent.id, ing_id, cnt, dst=res.tloc("area", current_area.id))

            res.events.append(Event(
                type="object_shattered",
                agent_id=agent.id,
                data={"obj_id": base_id, "salvage": salvage, "area_id": current_area.id},
            ))

        # if NPC dies, resolve like in attack
        if target_npc.hp <= 0:
            res.add_feedback(
                agent.id,
                f"{env.person_verbalized['subject_pronoun']} defeated {npc_name}.\n"
            )
            ...
            if not (tutorial_room and not tutorial_removed):
                env.curr_agents_state["npcs_killed"][agent.id].append(target_npc_id)
                target_npc_str = f"{get_def_id(target_npc_id)}_{target_npc.level}"
                if not target_npc_str in env.curr_agents_state["unique_npcs_killed"][agent.id]:
                    env.curr_agents_state["unique_npcs_killed"][agent.id].append(target_npc_str)

            if target_npc_id in current_area.npcs:
                current_area.npcs.remove(target_npc_id)

            # drop loot
            for loot_obj_id, count in (target_npc.inventory or {}).items():
                if count > 0:
                    if loot_obj_id in world.objects and (world.objects[loot_obj_id].category == "container" or world.objects[loot_obj_id].usage == "writable"):
                        instance_list = world.container_instances if world.objects[loot_obj_id].category == "container" else world.writable_instances
                        id_to_count = (
                            world.auxiliary["container_id_to_count"]
                            if world.objects[loot_obj_id].category == "container"
                            else world.auxiliary["writable_id_to_count"]
                        )
                        for _ in range(count):
                            new_instance = world.objects[loot_obj_id].create_instance(id_to_count[loot_obj_id])
                            instance_list[new_instance.id] = new_instance
                            id_to_count[loot_obj_id] += 1
                            world.auxiliary["obj_name_to_id"][new_instance.name] = new_instance.id
                            current_area.objects[new_instance.id] = current_area.objects.get(new_instance.id, 0) + 1
                            res.track_spawn(agent.id, new_instance.id, 1, dst=res.tloc("area", current_area.id))
                    else:
                        current_area.objects[loot_obj_id] = current_area.objects.get(loot_obj_id, 0) + count
                        res.track_spawn(agent.id, loot_obj_id, count, dst=res.tloc("area", current_area.id))
...
class DisassembleRule(BaseActionRule):
    name = "action_disassemble"
    verb = "disassemble"
    param_min = param_max = 1
    params = ["obj_name"]
    description = "Break down a craftable object (in hand or on the ground; not while equipped) into some of its original ingredients."
    ...
    def apply(self, ctx: RuleContext, res: RuleResult) -> None:
        ...
        if obj_name not in world.auxiliary["obj_name_to_id"]:
            res.add_feedback(agent.id, f"Cannot disassemble {obj_name}, not found in hand or on the ground.\n")
            return
        obj_id = world.auxiliary["obj_name_to_id"][obj_name]

        current_area = world.area_instances[env.curr_agents_state["area"][agent.id]]

        if obj_id in agent.equipped_items_in_limb and agent.equipped_items_in_limb[obj_id] > 0:
            res.add_feedback(agent.id, f"Cannot disassemble {obj_name}, it is currently equipped.\n")
            return
        ...
        if obj_id in agent.items_in_hands and agent.items_in_hands[obj_id] > 0:
            src_loc = res.tloc("hand", agent.id)
            loc_label = "hand"
        elif obj_id in current_area.objects and current_area.objects[obj_id] > 0:
            src_loc = res.tloc("area", current_area.id)
            loc_label = "ground"
        else:
            res.add_feedback(agent.id, f"Cannot disassemble {obj_name}, not found in hand or on the ground.\n")
        ...
        base_id = get_def_id(obj_id)
        if base_id not in world.objects:
            res.add_feedback(agent.id, f"Cannot disassemble {obj_name}, unknown object.\n")
            return
        obj_def = world.objects[base_id]
        ingredients = obj_def.craft_ingredients
        if not ingredients:
            res.add_feedback(agent.id, f"Cannot disassemble {obj_name}, it has no crafting recipe.\n")
            return

        # compute salvage yields
        salvage: dict[str, int] = {}
        for ing_id, req in ingredients.items():
            try:
                req_amt = float(req)
            ...
            if req_amt >= 2:
                ...
                # fractional (<1) or zero yields nothing
                out = 0
            if out > 0:
                salvage[ing_id] = int(out)
        ...
        if loc_label == "hand":
            ...
            current_area.objects[obj_id] -= 1
            if current_area.objects[obj_id] <= 0:
                del current_area.objects[obj_id]
        res.track_consume(agent.id, obj_id, 1, src=src_loc)
        ...
        if salvage:
            parts = [f"{count} {world.objects[ing_id].name if ing_id in world.objects else ing_id}" for ing_id, count in salvage.items()]
            gained = ", ".join(parts)
            res.add_feedback(agent.id, f"{env.person_verbalized['subject_pronoun']} disassembled {obj_name} and recovered {gained}.\n")
        else:
            res.add_feedback(agent.id, f"{env.person_verbalized['subject_pronoun']} disassembled {obj_name} but recovered nothing useful.\n")

        res.events.append(Event(
            type="object_disassembled",
            agent_id=agent.id,
            data={"obj_id": base_id, "yield": salvage, "area_id": current_area.id},
        ))

class PickpocketRule(BaseActionRule):
    name = "action_pickpocket"
    verb = "pickpocket"
    param_min = param_max = 1
    params = ["npc_name"]
    description = "Attempt to stealthily steal from an NPC in the current area."

    def apply(self, ctx: RuleContext, res: RuleResult) -> None:
        env, world, agent = ctx.env, ctx.world, ctx.agent
        npc_name = ctx.params[0]

        current_area = world.area_instances[env.curr_agents_state["area"][agent.id]]

        if npc_name not in world.auxiliary.get("npc_name_to_id", {}):
            res.add_feedback(agent.id, f"Cannot pickpocket {npc_name}, not found in current area.\n")
            return
        ...
        hands_fullness = sum(agent.items_in_hands.values())
        chance = 0.4
        if hands_fullness == 0:  # if empty hands, then higher chance
            chance += 0.1
        if hands_fullness >= 2:  # if full hands, then lower chance
            chance -= 0.2
        if getattr(npc, "role", None) == "merchant":  # if merchant, harder to pickpocket
            chance -= 0.15
        if chance < 0.0:
            chance = 0.0
        if chance > 1.0:
        ...
        if roll < chance:
            # success
            free_hand = hands_fullness <= 1
            stolen_desc = None

            coin_id = "obj_coin"
            npc_coins = int(getattr(npc, "coins", 0))
            if npc_coins > 0:
                steal_amt = min(npc_coins, env.rng.randint(1, 5))
                npc.coins = npc_coins - steal_amt
                if free_hand:
                    agent.items_in_hands[coin_id] = agent.items_in_hands.get(coin_id, 0) + steal_amt
                    res.track_spawn(agent.id, coin_id, steal_amt, dst=res.tloc("hand", agent.id))
                    loc_str = "hand"
                else:
                    current_area.objects[coin_id] = current_area.objects.get(coin_id, 0) + steal_amt
                    res.track_spawn(agent.id, coin_id, steal_amt, dst=res.tloc("area", current_area.id))
                    loc_str = "ground"
                res.add_feedback(agent.id, f"{env.person_verbalized['subject_pronoun']} pickpocketed {npc_name} and stole {steal_amt} coin(s) to {loc_str}.\n")
                stolen_desc = {"obj_id": coin_id, "amount": steal_amt}
            else:
                # steal one item unit from npc inventory if available
                inv_items = [(oid, cnt) for oid, cnt in (getattr(npc, "inventory", {}) or {}).items() if int(cnt) > 0]
                if not inv_items:
                    res.add_feedback(agent.id, f"{env.person_verbalized['subject_pronoun']} tried to pickpocket {npc_name}, but they had nothing to steal.\n")
                    res.events.append(Event(
                        type="npc_pickpocketed",
                        agent_id=agent.id,
                        data={"npc_id": npc_id, "what_was_stolen": {"obj_id": None, "amount": 0}, "area_id": current_area.id},
                    ))
                    return

                base_id, _ = env.rng.choice(inv_items)
                npc.inventory[base_id] -= 1
                if npc.inventory[base_id] <= 0:
                    del npc.inventory[base_id]

                obj_def = world.objects.get(base_id)
                if obj_def and (obj_def.category == "container" or obj_def.usage == "writable"):
                    # instance-based object: create a fresh instance
                    if obj_def.category == "container":
                        instance_list = world.container_instances
                        id_to_count = world.auxiliary.setdefault("container_id_to_count", {})
                    else:
                    ...
                    if free_hand:
                        agent.items_in_hands[new_instance.id] = agent.items_in_hands.get(new_instance.id, 0) + 1
                        res.track_spawn(agent.id, new_instance.id, 1, dst=res.tloc("hand", agent.id))
                        loc_str = "hand"
                    else:
                        current_area.objects[new_instance.id] = current_area.objects.get(new_instance.id, 0) + 1
                        res.track_spawn(agent.id, new_instance.id, 1, dst=res.tloc("area", current_area.id))
                        loc_str = "ground"
                    disp_name = getattr(obj_def, "name", new_instance.id)
                    res.add_feedback(agent.id, f"{env.person_verbalized['subject_pronoun']} pickpocketed {npc_name} and stole 1 {disp_name} to {loc_str}.\n")
                    stolen_desc = {"obj_id": base_id, "amount": 1}
                else:
                    # normal object
                    if free_hand:
                        agent.items_in_hands[base_id] = agent.items_in_hands.get(base_id, 0) + 1
                        res.track_spawn(agent.id, base_id, 1, dst=res.tloc("hand", agent.id))
                        loc_str = "hand"
                    else:
                        current_area.objects[base_id] = current_area.objects.get(base_id, 0) + 1
                        res.track_spawn(agent.id, base_id, 1, dst=res.tloc("area", current_area.id))
                        loc_str = "ground"
                    disp_name = world.objects[base_id].name if base_id in world.objects else base_id
                    res.add_feedback(agent.id, f"{env.person_verbalized['subject_pronoun']} pickpocketed {npc_name} and stole 1 {disp_name} to {loc_str}.\n")
                    stolen_desc = {"obj_id": base_id, "amount": 1}
            ...
            res.events.append(Event(
                type="npc_pickpocketed",
                agent_id=agent.id,
                data={"npc_id": npc_id, "what_was_stolen": stolen_desc, "area_id": current_area.id},
            ))
        else:
            # failure and retaliation
            damage = max(1, int(getattr(npc, "attack_power", 0)) - int(getattr(agent, "defense", 0)))
            agent.hp -= damage
            res.add_feedback(agent.id, f"{env.person_verbalized['subject_pronoun']} tried to pickpocket {npc_name} but was caught and took {damage} damage.\n")
            res.events.append(Event(
                type="agent_caught_stealing",
                agent_id=agent.id,
                data={"npc_id": npc_id, "damage": damage, "area_id": current_area.id},
            ))
