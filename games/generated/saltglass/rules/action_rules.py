import math
from games.generated.saltglass.rule import BaseActionRule, RuleContext, RuleResult, Event
...
class ListenRule(BaseActionRule):
    name = "action_listen"
    verb = "listen"
    param_min = 1
    param_max = 1
    params = ["target"]
    description = (
        "Listen to the saltglass in a toppled observatory, or while holding a glass object, "
        "to receive a cryptic hint about one nearby hidden object, locked path, or hostile NPC rhythm."
    )

    OBSERVATORY_AREA_NAMES = {"library", "orrery", "spire"}
    GLASS_NAME_TOKENS = ("glass", "lens", "saltglass", "brine")
    GLASS_USAGE_TOKENS = {"writable"}
    GLASS_CATEGORY_TOKENS = {"material"}

    def _is_observatory_area(self, area) -> bool:
        area_name = str(getattr(area, "name", "") or "").strip().lower()
        return area_name in self.OBSERVATORY_AREA_NAMES

    def _is_glasslike_object_id(self, world, obj_id: str) -> bool:
        base_id = get_def_id(obj_id)
        if base_id in getattr(world, "objects", {}):
            obj = world.objects[base_id]
        elif obj_id in getattr(world, "writable_instances", {}):
            obj = world.writable_instances[obj_id]
        elif obj_id in getattr(world, "container_instances", {}):
            obj = world.container_instances[obj_id]
        else:
            return False

        name = str(getattr(obj, "name", "") or "").lower()
        desc = str(getattr(obj, "description", "") or "").lower()
        usage = str(getattr(obj, "usage", "") or "").lower()
        category = str(getattr(obj, "category", "") or "").lower()
        oid = str(getattr(obj, "id", "") or "").lower()

        if any(tok in name for tok in self.GLASS_NAME_TOKENS):
            return True
        if any(tok in desc for tok in self.GLASS_NAME_TOKENS):
            return True
        if any(tok in oid for tok in self.GLASS_NAME_TOKENS):
            return True
        if usage in self.GLASS_USAGE_TOKENS and any(tok in name or tok in desc or tok in oid for tok in ("paper", "lens", "glass")):
            return True
        if category in self.GLASS_CATEGORY_TOKENS and any(tok in name or tok in desc or tok in oid for tok in self.GLASS_NAME_TOKENS):
            return True
        return False

    def _held_glasslike_objects(self, world, agent) -> list[str]:
        held = []
        for obj_id in (agent.items_in_hands or {}).keys():
            if self._is_glasslike_object_id(world, obj_id):
                held.append(obj_id)
        return held

    def _cryptic_object_hint(self, world, current_area) -> tuple[str, dict]:
        if not current_area.objects:
            return "", {}

        candidates = []
        for obj_id, count in current_area.objects.items():
            if int(count) <= 0:
                continue
            if obj_id in world.writable_instances:
                obj = world.writable_instances[obj_id]
            elif obj_id in world.container_instances:
                obj = world.container_instances[obj_id]
            elif get_def_id(obj_id) in world.objects:
                obj = world.objects[get_def_id(obj_id)]
            else:
                continue
            candidates.append((obj_id, obj))

        if not candidates:
            return "", {}

        obj_id, obj = sorted(candidates, key=lambda x: str(x[0]))[0]
        obj_name = str(getattr(obj, "name", "something"))
        category = str(getattr(obj, "category", "object"))
        usage = str(getattr(obj, "usage", ""))
        area_name = str(getattr(current_area, "name", "here"))

        lines = [
            f"The saltglass sings of {area_name}: beneath the white hush lies {obj_name}.",
            f"A thin ringing in the glass points toward {obj_name}, a {category} left half-buried in salt.",
            f"The shard remembers a shape nearby — {obj_name}, waiting in the flats like a forgotten {usage or category}.",
        ]
        text = lines[sum(ord(c) for c in str(obj_id)) % len(lines)]
        return text, {
            "hint_type": "nearby_object",
            "obj_id": obj_id,
            "area_id": current_area.id,
        }

    def _cryptic_locked_path_hint(self, world, current_area) -> tuple[str, dict]:
        locked = []
        for neighbor_id, path in (current_area.neighbors or {}).items():
            if getattr(path, "locked", False):
                locked.append((neighbor_id, path))

        if not locked:
            return "", {}

        neighbor_id, path = sorted(locked, key=lambda x: str(x[0]))[0]
        neighbor = world.area_instances.get(neighbor_id)
        neighbor_name = getattr(neighbor, "name", neighbor_id)
        key_id = getattr(path, "object_to_unlock", None)
        key_name = "a key"
        if key_id and key_id in world.objects:
            key_name = world.objects[key_id].name

        lines = [
            f"The saltglass hums toward {neighbor_name}; a sealed way sleeps there and wakes for {key_name}.",
            f"A brittle note inside the glass says the road to {neighbor_name} is still bound shut.",
            f"The listening tone bends toward {neighbor_name}; salt remembers a locked passage and the shape of {key_name}.",
        ]
        text = lines[sum(ord(c) for c in str(neighbor_id)) % len(lines)]
        return text, {
            "hint_type": "locked_path",
            "to_area": neighbor_id,
            "from_area": current_area.id,
            "key_id": key_id,
        }

    def _cryptic_enemy_rhythm_hint(self, world, current_area) -> tuple[str, dict]:
        candidates = []
        for npc_id in (current_area.npcs or []):
            npc = world.npc_instances.get(npc_id)
            if not npc or not getattr(npc, "enemy", False):
                continue
            rhythm = list(getattr(npc, "combat_pattern", []) or [])
            if not rhythm:
                continue
            candidates.append((npc_id, npc, rhythm))

        if not candidates:
            return "", {}

        npc_id, npc, rhythm = sorted(candidates, key=lambda x: str(x[0]))[0]
        rhythm_preview = " → ".join(rhythm[:4])
        lines = [
            f"The saltglass clicks in a hostile measure around {npc.name}: {rhythm_preview}.",
            f"A dry chime carries {npc.name}'s habit: {rhythm_preview}. Listen, and strike between the beats.",
            f"The shard catches the rhythm of {npc.name}: {rhythm_preview}. Salt remembers how violence repeats.",
        ]
        text = lines[sum(ord(c) for c in str(npc_id)) % len(lines)]
        return text, {
            "hint_type": "enemy_rhythm",
            "npc_id": npc_id,
            "area_id": current_area.id,
            "rhythm": rhythm[:],
        }
    ...
    def apply(self, ctx: RuleContext, res: RuleResult) -> None:
        ...
        target = str(ctx.params[0]).strip().lower()

        if target != "saltglass":
            res.add_feedback(
                agent.id,
                "Invalid target for listen. Use: listen saltglass.\n"
            )
            return
        ...
        in_observatory = self._is_observatory_area(current_area)
        held_glass = self._held_glasslike_objects(world, agent)

        if not in_observatory and not held_glass:
            res.add_feedback(
                agent.id,
                "Cannot listen to the saltglass here. You need to stand in a toppled observatory or hold something of glass and brine.\n"
            )
            return

        hint_candidates = []

        obj_hint = self._cryptic_object_hint(world, current_area)
        if obj_hint[0]:
            hint_candidates.append(obj_hint)

        locked_hint = self._cryptic_locked_path_hint(world, current_area)
        if locked_hint[0]:
            hint_candidates.append(locked_hint)

        rhythm_hint = self._cryptic_enemy_rhythm_hint(world, current_area)
        if rhythm_hint[0]:
            hint_candidates.append(rhythm_hint)

        if not hint_candidates:
            res.add_feedback(
                agent.id,
                "You listen to the saltglass, but it only answers with a long, empty hush.\n"
            )
            res.events.append(Event(
                type="saltglass_listened",
                agent_id=agent.id,
                data={
                    "area_id": current_area_id,
                    "hint_type": "none",
                    "used_observatory": in_observatory,
                    "used_glass_object_ids": held_glass,
                },
            ))
            return

        idx_seed = env.steps + sum(ord(c) for c in agent.id) + len(current_area.objects) + len(current_area.npcs)
        text, data = hint_candidates[idx_seed % len(hint_candidates)]

        res.add_feedback(
            agent.id,
            f"{env.person_verbalized['subject_pronoun']} listened to the saltglass.\n{text}\n"
        )
        payload = {
            "area_id": current_area_id,
            "used_observatory": in_observatory,
    ...
    def apply(self, ctx: RuleContext, res: RuleResult) -> None:
        ...
        if target_npc_instance.role == "merchant":
            ...
            for item_id, count in target_npc_instance.inventory.items():
                ...
                lines += f"{count} {item_name} (each for {value_each} {coin_name}s), "
            lines = lines.rstrip(", ") + "\n"
            feedback_str += lines
...
class BuyRule(BaseActionRule):
    ...
    description = "Buy a specified amount of an object from a merchant NPC using currency."
    ...
    def apply(self, ctx: RuleContext, res: RuleResult) -> None:
        ...
        if coin_have < total_cost:
            res.add_feedback(
                agent.id,
                f"Cannot buy {buy_amount} {obj_name}. Need {total_cost} {world.objects['obj_coin'].name if 'obj_coin' in world.objects else 'coin'}s but only have {coin_have}.\n"
            )
            return
        ...
        if remain_pay != 0:
            res.add_feedback(agent.id, f"Cannot buy {obj_name}. Failed to pay {world.objects['obj_coin'].name if 'obj_coin' in world.objects else 'coin'}s.\n")
            return
        ...
        res.add_feedback(
            agent.id,
            f"{env.person_verbalized['subject_pronoun']} bought {buy_amount} {obj_name} from {npc_name} for {total_cost} {world.objects['obj_coin'].name if 'obj_coin' in world.objects else 'coin'}s. "
            f"It is now on the ground.\n"
        )
        res.events.append(Event(
            type="object_bought",
...
class SellRule(BaseActionRule):
    ...
    def apply(self, ctx: RuleContext, res: RuleResult) -> None:
        ...
        if sell_unit_price > 0:
            ...
            if max_affordable <= 0:
                res.add_feedback(agent.id, f"Cannot sell {obj_name}. {npc_name} does not have enough {world.objects['obj_coin'].name if 'obj_coin' in world.objects else 'coin'}s to buy.\n")
                return
        ...
        res.add_feedback(
            agent.id,
            f"{env.person_verbalized['subject_pronoun']} sold {sell_amount} {obj_name} to {npc_name} for {total_gain} {world.objects['obj_coin'].name if 'obj_coin' in world.objects else 'coin'}s. "
            f"The {world.objects['obj_coin'].name if 'obj_coin' in world.objects else 'coin'}s are now on the ground.\n"
        )
        res.events.append(Event(
            type="object_sold",
            agent_id=agent.id,
...
class RefractRule(BaseActionRule):
    name = "action_refract"
    verb = "refract"
    param_min = param_max = 1
    params = ["glass_object_name"]
    description = (
        "Hold up a non-degraded glass object to refract light and reveal hidden details "
        "in the current area: locked-path key requirements, concealed objects, or NPC inventory."
    )

    GLASS_NAME_TOKENS = ("glass", "lens", "saltglass", "brine")

    def _is_glasslike(self, world, obj_id: str) -> bool:
        base_id = get_def_id(obj_id)
        if base_id in getattr(world, "objects", {}):
            obj = world.objects[base_id]
        elif obj_id in getattr(world, "writable_instances", {}):
    ...
    def _is_degraded(self, env, agent_id: str) -> bool:
        glaze = env.curr_agents_state.get("salt_glaze", {}).get(agent_id, {})
        return bool(glaze.get("active"))

    def apply(self, ctx: RuleContext, res: RuleResult) -> None:
        env, world, agent = ctx.env, ctx.world, ctx.agent
        glass_name = ctx.params[0]

        current_area_id = env.curr_agents_state["area"][agent.id]
        current_area = world.area_instances[current_area_id]

        # resolve the glass object in hand
        if glass_name not in world.auxiliary.get("obj_name_to_id", {}):
            res.add_feedback(agent.id, f"Cannot refract with {glass_name}. Object not found.\n")
            return
        glass_id = world.auxiliary["obj_name_to_id"][glass_name]
        if glass_id not in agent.items_in_hands:
            res.add_feedback(agent.id, f"{glass_name} must be held in hand to refract.\n")
            return

        if not self._is_glasslike(world, glass_id):
            res.add_feedback(agent.id, f"{glass_name} is not a glass object. Refract requires glass or lens.\n")
            return

        # must not be salt-glazed
        if self._is_degraded(env, agent.id):
            res.add_feedback(
                agent.id,
                f"The salt glaze crusting {glass_name} scatters the light uselessly. "
                f"Reach an observatory to clear the glaze first.\n"
            )
            return

        # track utilization of the glass
        res.track_utilize(agent.id, glass_id, 1, src=res.tloc("hand", agent.id))

        # gather revelations
        reveals = []

        # 1) locked paths and their keys
        for neighbor_id, path in (current_area.neighbors or {}).items():
            if not getattr(path, "locked", False):
                continue
            neighbor = world.area_instances.get(neighbor_id)
            neighbor_name = getattr(neighbor, "name", neighbor_id) if neighbor else neighbor_id
            key_id = getattr(path, "object_to_unlock", None)
            if key_id and key_id in world.objects:
                key_name = world.objects[key_id].name
                reveals.append(f"The light bends toward {neighbor_name} — the lock answers to {key_name}.")
            else:
                reveals.append(f"A sealed passage toward {neighbor_name} glimmers faintly.")

        # 2) hidden objects in area (items with 0 < count, sorted by id, show first 3)
        shown = 0
        for obj_id in sorted(current_area.objects.keys()):
            if shown >= 3:
                break
            count = int(current_area.objects.get(obj_id, 0))
            if count <= 0:
                continue
            base = get_def_id(obj_id)
            if base in world.objects:
                obj_name = world.objects[base].name
                category = world.objects[base].category
                reveals.append(f"Refracted light catches {count} {obj_name} ({category}) half-buried here.")
                shown += 1

        # 3) merchant inventory peek
        for npc_id in (current_area.npcs or []):
            npc = world.npc_instances.get(npc_id)
            if not npc or getattr(npc, "role", None) != "merchant":
                continue
            inv_items = []
            for item_id, cnt in (getattr(npc, "inventory", {}) or {}).items():
                if item_id in world.objects:
                    inv_items.append(f"{cnt} {world.objects[item_id].name}")
            if inv_items:
                reveals.append(f"Through the glass, {npc.name}'s wares shimmer: {', '.join(inv_items)}.")

        if not reveals:
            res.add_feedback(
                agent.id,
                f"{env.person_verbalized['subject_pronoun']} held up {glass_name}, but the light reveals nothing new.\n"
            )
        else:
            body = "\n".join(reveals)
            res.add_feedback(
                agent.id,
                f"{env.person_verbalized['subject_pronoun']} held up {glass_name} and refracted the light.\n{body}\n"
            )
