...
class LightSourceDepletionStepRule(BaseStepRule):
    name = "light_source_depletion_step"
    description = (
        "Each step, any lit light source held or equipped by the player "
        "depletes fuel by 1. When fuel reaches 0 the light source is "
        "extinguished and the player enters darkness. In darkness the "
        "observation hides item and NPC details, and enemy ambush "
        "probability is sharply increased."
    )
    priority = 4  # runs before active attacks (6) so darkness state is set first

    FUEL_DRAIN_PER_STEP = 1

    def apply(self, ctx: RuleContext, res: RuleResult) -> None:
        env, world = ctx.env, ctx.world

        env.curr_agents_state.setdefault("in_darkness", {})
        env.curr_agents_state.setdefault("light_fuel", {})

        for agent in env.agents:
            aid = agent.id
            env.curr_agents_state["in_darkness"].setdefault(aid, False)
            env.curr_agents_state["light_fuel"].setdefault(aid, {})

            if agent.hp <= 0:
                continue

            # If the area itself is lit, the agent is never in darkness here
            agent_area_id = env.curr_agents_state["area"].get(aid)
            agent_area = world.area_instances.get(agent_area_id)
            if agent_area and getattr(agent_area, "light", False):
                if env.curr_agents_state["in_darkness"].get(aid, False):
                    env.curr_agents_state["in_darkness"][aid] = False
                continue

            # Collect all light source object ids held in hand or equipped
            light_sources = []  # list of (obj_id, location)
            for obj_id in list(agent.items_in_hands.keys()):
                obj_def = world.objects.get(get_def_id(obj_id))
                if obj_def and getattr(obj_def, "light_source", False):
                    light_sources.append((obj_id, "hand"))
            for obj_id in list(agent.equipped_items_in_limb.keys()):
                obj_def = world.objects.get(get_def_id(obj_id))
                if obj_def and getattr(obj_def, "light_source", False):
                    light_sources.append((obj_id, "equip"))

            if not light_sources:
                # No light source at all -> darkness
                if not env.curr_agents_state["in_darkness"].get(aid, False):
                    env.curr_agents_state["in_darkness"][aid] = True
                    res.add_feedback(
                        aid,
                        f"{env.person_verbalized['subject_pronoun'].capitalize()} have no light source. "
                        f"Darkness engulfs {env.person_verbalized['object_pronoun']}. "
                        f"It's hard to see anything clearly.\n"
                    )
                    res.events.append(Event(
                        type="entered_darkness",
                        agent_id=aid,
                        data={"reason": "no_light_source"},
                    ))
                continue

            # Track fuel per individual light source instance
            fuel_state = env.curr_agents_state["light_fuel"][aid]
            any_lit = False

            for obj_id, location in light_sources:
                def_id = get_def_id(obj_id)
                ...
                # Initialise fuel for this specific item if not tracked yet
                if obj_id not in fuel_state:
                    fuel_state[obj_id] = max_fuel

                current_fuel = fuel_state[obj_id]

                if current_fuel <= 0:
                    # Already depleted, skip
                    continue

                # Drain fuel
                current_fuel = max(0, current_fuel - self.FUEL_DRAIN_PER_STEP)
                fuel_state[obj_id] = current_fuel

                if current_fuel > 0:
                    any_lit = True
                    # Warn when fuel is getting low
                    if current_fuel <= 5:
                        light_name = obj_def.name
                        res.add_feedback(
                            aid,
                            f"{env.person_verbalized['possessive_adjective'].capitalize()} "
                            f"{light_name} flickers — only {current_fuel} fuel remaining.\n"
                        )
                else:
                    # Just ran out
                    light_name = obj_def.name
                    res.add_feedback(
                        aid,
                        f"{env.person_verbalized['possessive_adjective'].capitalize()} "
                        f"{light_name} sputters and dies. The fuel is exhausted.\n"
                    )
                    res.events.append(Event(
                        type="light_source_depleted",
                        agent_id=aid,
                        data={"obj_id": obj_id, "location": location},
                    ))

            # Update darkness state
            was_in_darkness = env.curr_agents_state["in_darkness"].get(aid, False)
            now_in_darkness = not any_lit

            env.curr_agents_state["in_darkness"][aid] = now_in_darkness

            if now_in_darkness and not was_in_darkness:
                res.add_feedback(
                    aid,
                    f"Darkness engulfs {env.person_verbalized['object_pronoun']}. "
                    f"Without a working light source, {env.person_verbalized['subject_pronoun']} "
                    f"can barely make out {env.person_verbalized['possessive_adjective']} surroundings. "
                    f"Find fuel or a new light source!\n"
                )
                res.events.append(Event(
                    type="entered_darkness",
                    agent_id=aid,
                    data={"reason": "fuel_exhausted"},
                ))
            elif not now_in_darkness and was_in_darkness:
                res.add_feedback(
                    aid,
                    f"Light returns! {env.person_verbalized['subject_pronoun'].capitalize()} "
                    f"can see clearly again.\n"
                )
                res.events.append(Event(
                    type="left_darkness",
                    agent_id=aid,
            ...
            # Clean up fuel entries for items no longer held
            held_ids = set(agent.items_in_hands.keys()) | set(agent.equipped_items_in_limb.keys())
            for tracked_id in list(fuel_state.keys()):
                if tracked_id not in held_ids:
                    del fuel_state[tracked_id]
    ...
    def apply(self, ctx: RuleContext, res: RuleResult) -> None:
        ...
        for agent in env.agents:
            ...
            for npc_id, combat_state in list(active_combats.items()):
                ...
                # check if this NPC is stunned (from furniture shove)
                stunned_npcs = env.curr_agents_state.get("stunned_npcs", {})
                if stunned_npcs.get(npc_id, 0) > 0:
                    stunned_npcs[npc_id] -= 1
                    if stunned_npcs[npc_id] <= 0:
                        del stunned_npcs[npc_id]
                    res.add_feedback(
                        agent.id,
                        f"{npc_instance.name} is stunned and cannot act this round!\n"
                    )
    ...
    def apply(self, ctx: RuleContext, res: RuleResult) -> None:
        ...
        for agent in env.agents:
            ...
            for npc_id in enemy_npcs[agent_area].copy():
                ...
                # sharply increase ambush probability when agent is in darkness
                agent_in_darkness = env.curr_agents_state.get("in_darkness", {}).get(agent.id, False)
                if agent_in_darkness:
                    base_prob = min(1.0, base_prob * 4.0)
...
class CorpseCorruptionStepRule(BaseStepRule):
    name = "corpse_corruption_step"
    description = (
        "Areas accumulate corruption when enemy corpses (defeated NPCs) are "
        "left in them. Once corruption crosses a threshold the area spawns "
        "mutated, stronger variants of previously defeated enemies."
    )
    priority = 8  # runs after death/respawn (7) but before world expansion (100)

    # Corruption rises by this amount per corpse per step
    CORRUPTION_PER_CORPSE_PER_STEP = 1
    # Threshold at which the area starts spawning mutants
    CORRUPTION_THRESHOLD = 10
    # Steps between successive mutant spawns once threshold is crossed
    SPAWN_COOLDOWN = 5
    # Stat multiplier for mutated variants
    MUTANT_HP_MULTIPLIER = 1.5
    MUTANT_ATK_MULTIPLIER = 1.4

    def apply(self, ctx: RuleContext, res: RuleResult) -> None:
        env, world = ctx.env, ctx.world

        # --- initialise persistent state ---
        area_corpses: Dict[str, List[str]] = env.curr_agents_state.setdefault(
            "area_corpses", {}
        )
        area_corruption: Dict[str, int] = env.curr_agents_state.setdefault(
            "area_corruption", {}
        )
        corruption_spawn_cd: Dict[str, int] = env.curr_agents_state.setdefault(
            "corruption_spawn_cooldown", {}
        )

        # --- 1. record fresh corpses from this step's kill events ---
        for ev in res.events:
            if getattr(ev, "type", None) == "npc_killed":
                data = getattr(ev, "data", {}) or {}
                npc_id = data.get("npc_id")
                kill_area = data.get("area_id")
                if not npc_id or not kill_area:
                    continue
                # only track enemy NPCs
                npc_inst = world.npc_instances.get(npc_id)
                if npc_inst is None:
                    # already removed; check prototype
                    base_id = get_def_id(npc_id)
                    proto = world.npcs.get(base_id)
                    if proto is None or not getattr(proto, "enemy", False):
                        continue
                elif not getattr(npc_inst, "enemy", True):
                    continue

                area_corpses.setdefault(kill_area, []).append(get_def_id(npc_id))

        # --- 2. tick corruption for every area that has corpses ---
        for area_id, corpse_list in list(area_corpses.items()):
            if not corpse_list:
                continue
            n_corpses = len(corpse_list)
            old_corruption = area_corruption.get(area_id, 0)
            new_corruption = old_corruption + n_corpses * self.CORRUPTION_PER_CORPSE_PER_STEP
            area_corruption[area_id] = new_corruption

            # warn agents in the area when corruption first crosses 50 %
            half_threshold = self.CORRUPTION_THRESHOLD // 2
            if old_corruption < half_threshold <= new_corruption:
                for agent in env.agents:
                    if env.curr_agents_state["area"].get(agent.id) == area_id:
                        ...
                        res.add_feedback(
                            agent.id,
                            f"A foul stench rises from the unburned corpses in {area_name}. "
                            f"The area is becoming corrupted.\n"
                        )

            # warn when threshold is crossed
            if old_corruption < self.CORRUPTION_THRESHOLD <= new_corruption:
                for agent in env.agents:
                    if env.curr_agents_state["area"].get(agent.id) == area_id:
                        area_name = world.area_instances[area_id].name if area_id in world.area_instances else area_id
                        res.add_feedback(
                            agent.id,
                            f"The corruption in {area_name} has reached critical levels! "
                            f"Mutated creatures are beginning to emerge from the corpses!\n"
                        )

        # --- 3. spawn mutants in corrupted areas ---
        npc_id_to_count = world.auxiliary.setdefault("npc_id_to_count", {})

        for area_id, corruption in list(area_corruption.items()):
            if corruption < self.CORRUPTION_THRESHOLD:
                continue
            corpse_list = area_corpses.get(area_id, [])
            if not corpse_list:
                continue

            # cooldown management
            cd = corruption_spawn_cd.get(area_id, 0)
            if cd > 0:
                corruption_spawn_cd[area_id] = cd - 1
                continue

            # pick a random corpse type to mutate
            base_npc_id = env.rng.choice(corpse_list)
            proto = world.npcs.get(base_npc_id)
            if proto is None or not getattr(proto, "enemy", False):
                continue
            ...
            level = area.level
            idx = npc_id_to_count.get(base_npc_id, 0)
            new_npc = proto.create_instance(idx, level, world.objects, env.rng)
            npc_id_to_count[base_npc_id] = idx + 1

            # buff the mutant
            new_npc.hp = int(new_npc.hp * self.MUTANT_HP_MULTIPLIER)
            # attack_power is a regular attribute on instances (slope is frozen only)
            object.__setattr__(new_npc, "attack_power",
                               int(new_npc.attack_power * self.MUTANT_ATK_MULTIPLIER))
            # prefix name so the player can tell it apart
            mutant_name = f"mutated_{new_npc.name}"
            old_name = new_npc.name
            new_npc.name = mutant_name

            world.npc_instances[new_npc.id] = new_npc
            world.auxiliary.setdefault("npc_name_to_id", {})[mutant_name] = new_npc.id
            # also keep the original name mapping in case of collisions
            if old_name not in world.auxiliary.get("npc_name_to_id", {}):
                world.auxiliary["npc_name_to_id"][old_name] = new_npc.id
            area.npcs.append(new_npc.id)

            corruption_spawn_cd[area_id] = self.SPAWN_COOLDOWN
            ...
            for agent in env.agents:
                if env.curr_agents_state["area"].get(agent.id) == area_id:
                    res.add_feedback(
                        agent.id,
                        f"A {mutant_name} has risen from the rotting corpses! "
                        f"It looks stronger than the original.\n"
                    )
            ...
            res.events.append(Event(
                ...
                data={
                    "npc_id": new_npc.id,
                    "base_npc_id": base_npc_id,
                    "area_id": area_id,
                    "corruption": corruption,
                },
            ))

class QuarantineLockdownStepRule(BaseStepRule):
    name = "quarantine_lockdown_step"
    description = (
        "Areas containing unburned corpses accumulate a quarantine lockdown "
        "timer each step. Once the timer crosses a threshold, the facility's "
        "emergency quarantine protocol activates: all paths leading out of "
        "the area lock and containers in the area become inaccessible. "
        "Players must craft or find a master_keycard to override the lockdown. "
        "Burning corpses resets the timer and prevents lockdown."
    )
    priority = 9  # after corpse corruption (8), before world expansion (100)

    # Steps with corpses before lockdown triggers
    LOCKDOWN_THRESHOLD = 8
    # Timer increment per step per area that has corpses
    TIMER_INCREMENT = 1
    # Warning thresholds (fraction of LOCKDOWN_THRESHOLD)
    WARN_AT_HALF = True

    MASTER_KEYCARD_ID = "obj_master_keycard"
    ...
    def apply(self, ctx: RuleContext, res: RuleResult) -> None:
        ...
        lockdown_state: Dict[str, Any] = env.curr_agents_state.setdefault(
            "quarantine_lockdown", {}
        )
        # lockdown_state structure:
        # {
        #   area_id: {
        #     "timer": int,
        #     "active": bool,
        #     "locked_paths": [ {"from": area_id, "to": neighbor_id}, ... ],
        #     "locked_containers": [ container_id, ... ],
        #   }
        # }

        # --- 1. Auto-unlock: if an agent in a locked-down area holds a master keycard ---
        for agent in env.agents:
            if agent.hp <= 0:
                continue
            agent_area_id = env.curr_agents_state["area"][agent.id]
            area_lock = lockdown_state.get(agent_area_id)
            if not area_lock or not area_lock.get("active", False):
                continue

            # Check if agent has a master keycard in hand, inventory, or held containers
            keycard_id = self.MASTER_KEYCARD_ID
            keycard_location = None  # ("hand",) or ("inventory",) or ("container", ci)

            if agent.items_in_hands.get(keycard_id, 0) > 0:
                keycard_location = ("hand",)
            elif agent.inventory.container and agent.inventory.items.get(keycard_id, 0) > 0:
                keycard_location = ("inventory",)
            else:
                for oid in list(agent.items_in_hands.keys()):
                    if oid in world.container_instances:
                        ci = world.container_instances[oid]
                        if ci.inventory.get(keycard_id, 0) > 0:
                            keycard_location = ("container", ci)
                            break

            if keycard_location is None:
                continue

            # Consume the keycard
            if keycard_location[0] == "hand":
                agent.items_in_hands[keycard_id] -= 1
                if agent.items_in_hands[keycard_id] <= 0:
                    del agent.items_in_hands[keycard_id]
                res.track_consume(agent.id, keycard_id, 1, src=res.tloc("hand", agent.id))
            elif keycard_location[0] == "inventory":
                agent.inventory.items[keycard_id] -= 1
                if agent.inventory.items[keycard_id] <= 0:
                    del agent.inventory.items[keycard_id]
                res.track_consume(
                    agent.id, keycard_id, 1,
                    src=res.tloc("container", agent.inventory.container.id),
                )
            elif keycard_location[0] == "container":
                ci = keycard_location[1]
                ci.inventory[keycard_id] -= 1
                if ci.inventory[keycard_id] <= 0:
                    del ci.inventory[keycard_id]
                res.track_consume(agent.id, keycard_id, 1, src=res.tloc("container", ci.id))

            # Unlock all paths
            for path_info in area_lock.get("locked_paths", []):
                from_id = path_info["from"]
                to_id = path_info["to"]
                from_area = world.area_instances.get(from_id)
                to_area = world.area_instances.get(to_id)
                if from_area and to_id in from_area.neighbors:
                    from_area.neighbors[to_id].locked = False
                if to_area and from_id in to_area.neighbors:
                    to_area.neighbors[from_id].locked = False

            # Unlock containers
            for cid in area_lock.get("locked_containers", []):
                ci = world.container_instances.get(cid)
                if ci:
                    ci.extra.pop("quarantine_locked", None)

            area_name = world.area_instances[agent_area_id].name if agent_area_id in world.area_instances else agent_area_id
            res.add_feedback(
                agent.id,
                f"{env.person_verbalized['subject_pronoun'].capitalize()} swiped the master_keycard "
                f"through the security panel. The quarantine lockdown in {area_name} has been "
                f"overridden! Doors and containers are accessible again.\n"
            )
            res.events.append(Event(
                type="quarantine_lockdown_overridden",
                agent_id=agent.id,
                data={"area_id": agent_area_id},
            ))

            # Reset lockdown state for this area
            area_lock["active"] = False
            area_lock["timer"] = 0
            area_lock["locked_paths"] = []
            area_lock["locked_containers"] = []

        # --- 2. Tick lockdown timers for areas with corpses ---
        for area_id, corpse_list in list(area_corpses.items()):
            if not corpse_list:
                # If corpses were cleared (burned), reset timer
                if area_id in lockdown_state:
                    lock_info = lockdown_state[area_id]
                    if not lock_info.get("active", False):
                        lock_info["timer"] = 0
                continue

            lock_info = lockdown_state.setdefault(area_id, {
                "timer": 0,
                "active": False,
                "locked_paths": [],
                "locked_containers": [],
            ...
            if lock_info.get("active", False):
                # Already locked down, nothing more to do for timer
                continue

            old_timer = lock_info["timer"]
            new_timer = old_timer + self.TIMER_INCREMENT
            lock_info["timer"] = new_timer

            area_name = world.area_instances[area_id].name if area_id in world.area_instances else area_id

            # Warning at half threshold
            half = self.LOCKDOWN_THRESHOLD // 2
            if self.WARN_AT_HALF and old_timer < half <= new_timer:
                for agent in env.agents:
                    if env.curr_agents_state["area"].get(agent.id) == area_id:
                        res.add_feedback(
                            agent.id,
                            f"Warning lights flash in {area_name}. The facility's quarantine "
                            f"sensors are detecting biological contamination from the corpses. "
                            f"A lockdown may be imminent — burn the corpses or leave soon!\n"
                        )

            # Warning one step before lockdown
            if new_timer == self.LOCKDOWN_THRESHOLD - 1:
                for agent in env.agents:
                    if env.curr_agents_state["area"].get(agent.id) == area_id:
                        res.add_feedback(
                            agent.id,
                            f"CRITICAL: Quarantine protocol in {area_name} is about to engage! "
                            f"Burn the corpses NOW or find a master_keycard!\n"
                        )

            # --- 3. Trigger lockdown ---
            if new_timer >= self.LOCKDOWN_THRESHOLD:
                lock_info["active"] = True
                area = world.area_instances.get(area_id)
                if not area:
                    continue

                # Lock all paths out of this area
                locked_paths = []
                for neighbor_id, path in area.neighbors.items():
                    if not path.locked:
                        path.locked = True
                        locked_paths.append({"from": area_id, "to": neighbor_id})
                        # Lock the reverse path too
                        neighbor_area = world.area_instances.get(neighbor_id)
                        if neighbor_area and area_id in neighbor_area.neighbors:
                            neighbor_area.neighbors[area_id].locked = True

                lock_info["locked_paths"] = locked_paths

                # Lock containers in the area
                locked_containers = []
                for obj_id in list(area.objects.keys()):
                    if obj_id in world.container_instances:
                        ci = world.container_instances[obj_id]
                        ci.extra["quarantine_locked"] = True
                        locked_containers.append(obj_id)

                lock_info["locked_containers"] = locked_containers

                # Notify all agents in the area
                for agent in env.agents:
                    if env.curr_agents_state["area"].get(agent.id) == area_id:
                        res.add_feedback(
                            agent.id,
                            f"QUARANTINE LOCKDOWN ACTIVATED in {area_name}! "
                            f"Emergency blast doors have sealed all exits. "
                            f"Containers in the area are locked behind security panels. "
                            f"You need a master_keycard to override the lockdown.\n"
                        )

                # Also notify agents in neighboring areas
                for neighbor_id in [p["to"] for p in locked_paths]:
                    for agent in env.agents:
                        if env.curr_agents_state["area"].get(agent.id) == neighbor_id:
                            res.add_feedback(
                                agent.id,
                                f"You hear the heavy thud of blast doors slamming shut "
                                f"in the direction of {area_name}. A quarantine lockdown "
                                f"has been triggered.\n"
                            )

                res.events.append(Event(
                    type="quarantine_lockdown_activated",
                    agent_id=None,
                    data={
                        "area_id": area_id,
