...
from games.generated.quarantine.world import World, Object, NPC, Path, Area, Place, Container
from games.generated.quarantine.agent import Agent, Inventory, Action
from tools.logger import get_logger
from games.generated.quarantine.rule import RuleResult, RuleContext, ActionRuleEngine, BaseStepRule, BaseActionRule, DependencyTracker
from games.generated.quarantine.rules.reward_functions import RewardFunction, RewardBreakdown, DefaultRewardFunction
import games.generated.quarantine.rules.action_rules as action_rules
import games.generated.quarantine.rules.step_rules as step_rules
...
class AgentOdysseyEnv(gym.Env):
    ...
    def get_all_valid_actions(self, agent: Agent) -> list[str]:
        ...
        # --- Concoct (herb combinations) ---
        concoct_herb_ids = {"obj_green_herb", "obj_red_herb", "obj_blue_herb"}
        herb_counts: dict[str, int] = {}
        for hid in concoct_herb_ids:
            count = int(agent.items_in_hands.get(hid, 0))
            if agent.inventory.container:
                count += int(agent.inventory.items.get(hid, 0))
            for ci in held_containers:
                count += int(ci.inventory.get(hid, 0))
            if count > 0:
                herb_counts[hid] = count
        available_herbs = list(herb_counts.keys())
        for i, h1 in enumerate(available_herbs):
            for h2 in available_herbs[i:]:
                if h1 == h2 and herb_counts[h1] < 2:
                    continue
                h1_name = self.world.objects[h1].name
                h2_name = self.world.objects[h2].name
                actions["concoct"].append(f"concoct {h1_name} {h2_name}")

        # --- Shove (furniture at NPC in combat) ---
        active_combats_for_agent = self.curr_agents_state.get("active_combats", {}).get(agent.id, {})
        if active_combats_for_agent:
            furniture_in_area = [
                oid for oid in area_objects
                if get_def_id(oid) in self.world.objects
                and self.world.objects[get_def_id(oid)].category == "furniture"
            ]
            combat_npc_ids_in_area = [
                npc_id for npc_id in active_combats_for_agent
                if active_combats_for_agent[npc_id].get("area_id") == curr_area_id
                and npc_id in self.world.npc_instances
            ]
            for furn_id in furniture_in_area:
                furn_name = obj_id_to_name.get(furn_id, furn_id)
                for npc_id in combat_npc_ids_in_area:
                    npc_name = self.world.npc_instances[npc_id].name
                    actions["shove"].append(f"shove {furn_name} {npc_name}")

        # --- Eavesdrop ---
        # Can eavesdrop on any neighboring area when not in active combat
        active_combats_for_eavesdrop = self.curr_agents_state.get("active_combats", {}).get(agent.id, {})
        in_combat_here = any(
            cs.get("area_id") == curr_area_id
            for cs in active_combats_for_eavesdrop.values()
        )
        if not in_combat_here:
            for area in neighboring_areas:
                actions["eavesdrop"].append(f"eavesdrop {area}")

        # --- Barricade ---
        barricade_wood_ids = {"obj_wood_log", "obj_wood_plank"}
        barricade_wood_total = 0
        for wid in barricade_wood_ids:
            barricade_wood_total += int(agent.items_in_hands.get(wid, 0))
            if agent.inventory.container:
                barricade_wood_total += int(agent.inventory.items.get(wid, 0))
        if barricade_wood_total >= 3:
            for area in neighboring_areas:
                actions["barricade"].append(f"barricade {area}")

        # --- Burn corpses ---
        area_corpses = self.curr_agents_state.get("area_corpses", {})
        corpses_here = area_corpses.get(curr_area_id, [])
        if corpses_here:
            chem_count = int(agent.items_in_hands.get("obj_chemical", 0))
            rag_count = int(agent.items_in_hands.get("obj_rag", 0))
            if agent.inventory.container:
                chem_count += int(agent.inventory.items.get("obj_chemical", 0))
                rag_count += int(agent.inventory.items.get("obj_rag", 0))
            for ci in held_containers:
                chem_count += int(ci.inventory.get("obj_chemical", 0))
                rag_count += int(ci.inventory.get("obj_rag", 0))
            if chem_count >= 1 and rag_count >= 1:
                actions["burn corpses"].append("burn corpses")
        ...
        if fuel_have >= 1:
            for obj_id_hand in hand_objects:
                obj_def_hand = self.world.objects.get(get_def_id(obj_id_hand))
                if obj_def_hand and getattr(obj_def_hand, "light_source", False):
                    actions["refuel"].append(f"refuel {obj_id_to_name[obj_id_hand]}")
    ...
    def verbalize_obs(self, obs_time: str, obs_current_location: str, neighboring_areas: str, obs_items_in_hand: str, 
        ...
        if in_darkness:
            darkness_notice = (
                f"[It is pitch dark. {p['subject_pronoun'].capitalize()} can barely see anything. "
                f"Find and equip a light source to see clearly.]\n"
            )
            obs_objects = "indistinct shapes in the darkness"
            obs_npcs_and_agents = "shadowy figures lurking nearby" if obs_npcs_and_agents != "no one" else "no one (as far as you can tell)"
        else:
            darkness_notice = ""
        return f"""
...
"""
    ...
    def reset(self, from_config: bool = False) -> Dict[str, str]:
        if not from_config:
            ...
            self.curr_agents_state.setdefault("barricaded_paths", [])
            self.curr_agents_state.setdefault("stunned_npcs", {})

            # corpse corruption tracking
            self.curr_agents_state.setdefault("area_corpses", {})
            self.curr_agents_state.setdefault("area_corruption", {})
            self.curr_agents_state.setdefault("corruption_spawn_cooldown", {})

            # light source / darkness tracking
            self.curr_agents_state.setdefault("in_darkness", {})
            self.curr_agents_state.setdefault("light_fuel", {})
            for agent in self.agents:
                self.curr_agents_state["in_darkness"].setdefault(agent.id, False)
                self.curr_agents_state["light_fuel"].setdefault(agent.id, {})

            # quarantine lockdown tracking
            self.curr_agents_state.setdefault("quarantine_lockdown", {})
            ...
            # corpse corruption tracking (resume)
            self.curr_agents_state.setdefault("area_corpses", {})
            self.curr_agents_state.setdefault("area_corruption", {})
            self.curr_agents_state.setdefault("corruption_spawn_cooldown", {})

            # light source / darkness tracking (resume)
            self.curr_agents_state.setdefault("in_darkness", {})
            self.curr_agents_state.setdefault("light_fuel", {})
            for agent in self.agents:
                self.curr_agents_state["in_darkness"].setdefault(agent.id, False)
            ...
            # quarantine lockdown tracking (resume)
            self.curr_agents_state.setdefault("quarantine_lockdown", {})
        ...
        for agent in self.agents:
            ...
            agent_in_darkness = self.curr_agents_state.get("in_darkness", {}).get(agent.id, False)
