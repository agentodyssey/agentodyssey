...
from games.generated.mark.world import World, Object, NPC, Path, Area, Place, Container
from games.generated.mark.agent import Agent, Inventory, Action
from tools.logger import get_logger
from games.generated.mark.rule import RuleResult, RuleContext, ActionRuleEngine, BaseStepRule, BaseActionRule, DependencyTracker, Event
from games.generated.mark.rules.reward_functions import RewardFunction, RewardBreakdown, DefaultRewardFunction
import games.generated.mark.rules.action_rules as action_rules
import games.generated.mark.rules.step_rules as step_rules
...
class AgentOdysseyEnv(gym.Env):
    ...
    def get_all_valid_actions(self, agent: Agent) -> list[str]:
        ...
        # --- Defend (always available as a combat option) ---
        actions["defend"].append("defend")

        # --- Throw ---
        for obj_id in hand_objects:
            # cannot throw containers or writables
            if obj_id in self.world.container_instances or obj_id in self.world.writable_instances:
                continue
            ...
            for npc_id in curr_area.npcs:
                ...
                actions["throw"].append(f"throw {obj_name} {npc_name}")
        ...
        # --- Disassemble ---
        disassemble_candidates: set[str] = set()
        # from hands
        for obj_id in hand_objects:
            if obj_id in equipped_objects:
                continue
            base_id = get_def_id(obj_id)
            if base_id in self.world.objects and self.world.objects[base_id].craft_ingredients:
                disassemble_candidates.add(obj_id)
        # from ground
        for obj_id in area_objects:
            base_id = get_def_id(obj_id)
            if base_id in self.world.objects and self.world.objects[base_id].craft_ingredients:
                disassemble_candidates.add(obj_id)
        for obj_id in disassemble_candidates:
            if obj_id in obj_id_to_name:
                actions["disassemble"].append(f"disassemble {obj_id_to_name[obj_id]}")
...
"""
    ...
    def reset(self, from_config: bool = False) -> Dict[str, str]:
        ...
        if not from_config:
            ...
            self.world.export_graph(os.path.join(self.run_dir, "initial_world_graph.json"))
            self.steps = -1
        ...
        for agent in self.agents:
            if from_config:
                for agent_data in self.config["agents"]:
                    ...
                    logger.error(f"Corrupted config: Agent ID {agent.id} not found in config agents.")
    ...
    def step(self, action_strs: Dict[str, str]):
        ...
        for agent in self.agents:
            ...
            area = self.world.area_instances[agent_area]
            has_torch = any(get_def_id(oid) == "obj_torch" for oid in agent.items_in_hands.keys())
            can_see = bool(getattr(area, "light", True)) or has_torch

            if can_see:
                res.events.append(Event("area_lighted", agent.id, {"area_id": agent_area}))
                ...
                p = self.person_verbalized
                dark_msg = "It is too dark to see anything here.\n"
                obs_text = intro
                obs_text += (
                    f"\nCurrent Time: {current_info['time']}\n"
                    f"Current Location: {self.world.place_instances[current_info['area']['place']].name}, {current_info['area']['name']}\n\n"
                    f"{res.feedback[agent.id]}"
                    f"{dark_msg}\n"
                    f"{p['possessive_adjective'].capitalize()} level is {agent.level}.\n"
                    f"{p['possessive_adjective'].capitalize()} attack is at {agent.attack}.\n"
                    f"{p['possessive_adjective'].capitalize()} defense is at {agent.defense}.\n"
                    f"{p['possessive_adjective'].capitalize()} health is at {agent.hp}.\n"
                    f"{p['possessive_adjective'].capitalize()} experience is at {agent.xp}.\n\n"
                    f"Neighboring areas: {self.verbalize_areas(current_info['area']['neighbors'])}.\n"
                )
