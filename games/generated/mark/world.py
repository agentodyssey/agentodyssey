...
class NPC:
    ...
    coins: int
    base_hp: int  # default hp
    base_attack_power: int  # default attack power
    slope_hp: int  # hp increase per level
    slope_attack_power: int  # attack power increase per level
    ...
    def create_instance(self, index: int, level: int, objects: Dict[str, Object], 
        ...
        new_npc.level = level
        new_npc.hp = self.base_hp + self.slope_hp * (level - 1)
        new_npc.attack_power = self.base_attack_power + self.slope_attack_power * (level - 1)
        new_npc.coins = rng.randint(20 * level, 50 * level)
        for oid in self.inventory.keys():
            if oid in objects:
                count = rng.randint(0, level)
                if count > 0:
                  new_npc.inventory[oid] = count
        # combat_pattern is copied from prototype via deepcopy
        return new_npc
...
class World:
    ...
    def export_graph(self, filepath: Optional[str] = None) -> Dict[str, Any]:
        # export the current area graph layout to a JSON-serializable dict.
        connections = []
        seen_pairs = set()
        
        for area_id, area in self.area_instances.items():
            for neighbor_id, path in area.neighbors.items():
                # Create a canonical pair to avoid duplicates (A->B and B->A)
    ...
    def generate(world_definition: dict, seed: Optional[int] = None) -> World:
        ...
        try:
            ...
            # Check for predefined graph first, otherwise generate randomly
    ...
    def _parse_npc(npc: dict, rng: random.Random = None, objects: Dict[str, Object] = None) -> NPC:
        if rng is None:
            return NPC(
                ...
                base_hp=npc.get("base_hp", npc["hp"]),  # backward compatibility
                base_attack_power=npc.get("base_attack_power", npc["attack_power"]),
                slope_hp=npc["slope_hp"],
                slope_attack_power=npc["slope_attack_power"],
                level=npc["level"],
                quest=npc.get("quest", False),
                dialogue=npc.get("dialogue", ""),
                combat_pattern=npc.get("combat_pattern", []),
            )
                
        enemy = bool(npc.get("enemy", False))
        unique = bool(npc.get("unique", True))
        base_hp = npc.get("base_hp", 60) if enemy else npc.get("base_hp", 1000)
        hp = npc.get("base_hp", 60) if enemy else npc.get("base_hp", 1000)
        base_attack_power = npc.get("base_attack_power", 15) if enemy else npc.get("base_attack_power", 10)
        ...
        combat_pattern = npc.get("combat_pattern", [])
        if enemy and not combat_pattern:
            combat_pattern = ["attack", "attack", "wait", "defend"]
        ...
        for p in world_definition["entities"].get("places", []):
            ...
            for a in p.get("areas", []):
                ...
                areas[a["id"]] = Area(
                    ...
                    id=a["id"],
                    name=a["name"],
                    light=a.get("light", bool(rng.random() < 0.75)),
                    level=level,
                )
    ...
    def _load_predefined_graph(world_definition: dict, area_instances: Dict[str, Area]) -> bool:
        # load a human-defined graph from world_definition if present.
        logger = get_logger("WorldLogger")
    ...
    def _distribute_objects(world_definition: dict, rng: random.Random, area_instances: Dict[str, Area],
        ...
        for area in distributable_areas:
            ...
            k_materials = rng.randint(min(int(len(materials) * 2.5), int(distribution_k_by_area_level[level] * 2.5)), min(int(len(materials) * 5), int(distribution_k_by_area_level[level] * 5)))
    ...
    def _assign_npcs_to_areas(world_definition: dict, rng: random.Random, places: Dict[str, Place], area_instances: Dict[str, Area], 
        ...
        for home in distributable_area_list:
            enemy_count = rng.randint(1, home.level)  # make enemy count progress with area level
...
if __name__ == "__main__":
    world_definition = json.load(open("assets/world_definitions/generated/mark/default.json"))
